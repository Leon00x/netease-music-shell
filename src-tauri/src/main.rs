...
// 全宽悬浮标题栏：鼠标移到窗口顶部时浮现，整条可拖拽，右上角有控制按钮
const WINDOW_CONTROLS: &str = r#"
(function(){
  if (window.__nm_wc) return; window.__nm_wc = true;
  var CSS = ''
    + '#nm-wc-trigger{position:fixed;top:0;left:0;right:0;height:10px;z-index:2147483647;}'
  ...
  function boot(){
    ...
    trigger.addEventListener('mouseenter', function(){ bar.classList.add('on'); });
